...
public class Controlador extends IControlador {
	...
	public void altaUsuario(String nick, String nombre, String apellido, String email, LocalDate fechaNac,
		...
		try {
			...
			Profesor p = new Profesor(nick, apellido, email, nombre, fechaNac, imagen, biografia, descripcion, sitioWeb,
					i);
			tran.begin();
			em.persist(p);
			em.flush();
	...
	public void modificarDatos(String nombre, String apellido, LocalDate fechaNac, String imagen, String institucion,
	 ...
	 * 
	 * EntityManager em = emf.createEntityManager(); Cuponera cup = null;
	 * 
	 * try { em.getTransaction().begin(); cup = em.find(Cuponera.class, nombreCup);
	 * //busco cuponera seleccionada if(cup == null){ throw new
	 * Exception("La cuponera seleccionada no existe"); } cup.getData(); }catch
	 * (Exception ex) { if (em != null) { em.getTransaction().rollback(); } }
	 * finally { em.close(); }
	 * 
	 * //cup.getNombres(); //esto le paso al combobox? return null; }
	 */
	...
	public void altaActividadDepo(String nombreActividad, String nombreInsti, String desc, float dura, float costo,
		...
		// System.out.println(nombreActividad + nombreInsti + desc + dura + costo +
		// fechaAlta);

		// Institucion insti = new Institucion(em.find(Institucion)(Institucion.class,
		// nombreInsti));
		// EntityManager em = emf.createEntityManager();
	...
	public ArrayList<String> getNombreInstituciones() {
		...
		// List<Institucion> instis = em.createQuery("select i.nombreInst from
		// Institucion i").getResultList();
	...
	public ArrayList<DtCuponera> ListarCuponeras() {
		...
		for (Cuponera cup : list) {
			...
			DtCuponera DtCup = new DtCuponera(cup.getNombreCup(), cup.getDescripcion(), cup.getFechaInicio(),
					cup.getFechaFin(), cup.getDescuento(), cup.getFechaAlta(), cup.getCantClases(), actividades);
			l.add(DtCup);
		}
	...
	public ArrayList<String> getNombreCuponeras() {
		...
		// listaADevolver.addAll(em.createQuery("select c.nombreCup from Cuponeras
		// c").getResultList()); me tira error "cuponera no esta mapeada"
	...
	public ArrayList<String> consultarProfe(String nombreInsti) {
		...
		listaADevolver
				.addAll(em.createQuery("select p.nombre from Profesor p WHERE institucion =" + "'" + nombreInsti + "'")
						.getResultList());
		// System.out.println(listaADevolver);
		return listaADevolver;
	}

	public void darAltaClase(String nombreInsti, String nombreClase, LocalDateTime fechaInicio, String nombreProfe,
		...
		try {
			insti.darAltaClaseInsti(nombreInsti, nombreClase, fechaInicio, nombreProfe, sociosMin, sociosMax, URL,
					fechaAlta, this.emf);
		} catch (Exception e) {
