...
public class Controlador implements IControlador {
    ...
    public ArrayList<String> listaCuponerasRegistradas() {
        ...
        try {
            em.getTransaction().begin();
            consultaCuponera = (ArrayList<String>) em.createQuery("SELECT nombreCup FROM Cuponera").getResultList();// resultado
                                                                                                                    // =
                                                                                                                    // nombre
        } catch (Exception ex) {
            if (em != null) {
        ...
        for (int i = 0; i < consultaCuponera.size(); i++) {// itero y agrego nombres a la lista que voy a retornar
                                                           // ekisde
            String nombresCuponeras = (String) consultaCuponera.get(i); // obtengo el nombre en el q estoy parado casteo
                                                                        // a string xq consulta es de tipo List
    ...
    public void altaActividadDepo(String nombreActividad, String nombreInsti, String desc, float dura, float costo,
            LocalDateTime fechaAlta, ArrayList<String> catSeleccionadas) {
        EntityManager em = emf.createEntityManager();
    ...
    public ArrayList<String> consultarActividadDepo(String nombreInsti) {// ARREGLAR
        EntityManager em = emf.createEntityManager();
        ArrayList<String> listaADevolver = new ArrayList<String>();
        listaADevolver.addAll(em
                .createQuery(
                        "select a.nombreAct from ActividadDeportiva a WHERE institucion = " + "'" + nombreInsti + "'")
                .getResultList());
        em.clear();
        em.close();
        return listaADevolver;
    ...
    public ArrayList<String> consultarClases(String nombreActividad) {
        ...
        listaADevolver.addAll(
                em.createQuery("select nombre from Clase c where actividad_deportiva = '" + nombreActividad + "'")
                        .getResultList());
        em.clear();
        em.close();
        return listaADevolver;
    ...
    public String[] getUnaClase(String nombreClase) {
        ...
        cantMaxDeLaClase = em.createQuery("select cant_maxima from Clase c where nombre = '" + nombreClase + "'")
                .toString();
        cantMinDeLaClase = em.createQuery("select cant_minima from Clase c where nombre = '" + nombreClase + "'")
                .toString();
        horaIniDeLaClase = em.createQuery("select hora_inicio from Clase c where nombre = '" + nombreClase + "'")
                .toString();
        fechaDeLaClase = em.createQuery("select fecha from Clase c where nombre = '" + nombreClase + "'").toString();
        registroDeLaClase = em.createQuery("select fecha_registro from Clase c where nombre = '" + nombreClase + "'")
                .toString();
    ...
    public ArrayList<DtUsrKey> consultarProfe(String nombreInsti) {
        ...
        listaNick.addAll(
                em.createQuery("select p.nickname from Profesor p WHERE institucion =" + "'" + nombreInsti + "'")
                        .getResultList());
        listaEmails
                .addAll(em.createQuery("select p.email from Profesor p WHERE institucion =" + "'" + nombreInsti + "'")
                        .getResultList());
    ...
    public void darAltaClase(String nombreInsti, String nombreActiDepo, String nombreClase, LocalDateTime fechaInicio,
        ...
        try {
            em.getTransaction().begin();
            Clase claseDictada = profe.darAltaClaseProfe(nombreInsti, nombreClase, fechaInicio, sociosMin, sociosMax,
                    URL, fechaAlta, actiDepo);
    ...
    public HashMap<String, ArrayList<Object>> getHashInstisAndProfes() {
        ...
        for (int i = 0; i < listaDeInstis.size(); i++) {
            ...
            listaDeListas.add(this.consultarActividadDepo(listaDeInstis.get(i)));// a la primera posicion le paso un
                                                                                 // ArrayList con todos los nombres de
                                                                                 // las ActisDepo.
            listaDeListas.add(this.consultarProfe(listaDeInstis.get(i)));// a la 2nda posicion le paso un ArrayList de
                                                                         // DtUsrKey con todos los profes de esta Insti.
    ...
    public ArrayList<String> listaActividadesIngresada() {
        ...
        try {
            em.getTransaction().begin();
            consultaActividad = (ArrayList<String>) em
                    .createQuery("SELECT nombreAct FROM ActividadDeportiva where Estado = 0 ").getResultList();// nombre
                                                                                                               // Actividades
                                                                                                               // con
                                                                                                               // estado
                                                                                                               // Ingresada
        ...
        for (int i = 0; i < consultaActividad.size(); i++) {// itero y agrego nombres a la lista que voy a retornar
                                                            // ekisde
    ...
    public ArrayList<String> consultarActisAceptadas() {
        ...
        arrADevolver.addAll(
                em.createQuery("select a.nombreAct from ActividadDeportiva a WHERE estadoActual = 1").getResultList());
